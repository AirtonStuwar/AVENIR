...
export function useARendir() {
  ...
  const load = useCallback(async (p: number, estado: string | null, proyectoId: number | null, moneda: 'PEN' | 'USD' | null) => {
    setLoading(true)
    try {
      const res = await getARendir({ page: p, pageSize, role: userRole, userId: user?.id, estado, proyectoId, monedaFilter: moneda })
      setData(res.data)
      setTotal(res.total)
  ...
  return {
    ...
    setPage, setEstadoFilter: handleSetEstadoFilter, setProyectoFilter: handleSetProyectoFilter,
