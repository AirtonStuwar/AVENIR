...
export function OrdenCompraPDF({
  ...
  const retencionPct = isRxH || isLiberalidad || isOtros ? (solicitud.porcentaje_retencion ?? 0) : 0
  const retencion    = isRxH || isLiberalidad || isOtros ? (solicitud.monto_retencion ?? subtotal * retencionPct / 100) : 0
  const igv          = isRxH || isLiberalidad || isOtros || solicitud.aplica_igv === false ? 0 : subtotal * 0.18
  const total        = isRxH || isLiberalidad || isOtros ? subtotal - retencion : subtotal + igv
      ...
      <Page size="A4" style={S.page}>
        ...
        <View style={S.header}>
          ...
          <View style={S.headerCenter}>
            <Text style={S.headerTitle}>{isRxH ? 'RECIBO POR HONORARIOS' : isLiberalidad ? 'LIBERALIDAD' : isOtros ? 'OTROS' : 'ORDEN DE COMPRA O SERVICIO'}</Text>
            <Text style={S.headerSub}>{isRxH ? 'Documento de pago por servicios profesionales' : isLiberalidad ? 'Documento de pago por liberalidad' : isOtros ? 'Documento de pago' : 'Documento de solicitud de adquisición'}</Text>
          </View>
        ...
        <View style={S.totalsWrap}>
          <View style={S.totalsInner}>
            ...
            <View style={S.totalFinal}>
              <Text style={S.totalFinalText}>{isRxH || isLiberalidad || isOtros ? 'MONTO NETO A PAGAR' : 'TOTAL'}</Text>
