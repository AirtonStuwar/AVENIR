...
**Descarga masiva en ZIP:** botón "Descargar todos" en la tarjeta Documentos, visible para EVALUADOR/VISUALIZADOR/ADMIN cuando hay al menos un documento adjunto. Usa `JSZip` (mismo patrón que `SolicitudArchivos.tsx`) — la carpeta/ZIP se nombra con `cliente_nombre` (la persona a la que se le devuelve el dinero), y cada archivo dentro usa el label del tipo de documento (`Sustento.pdf`, `Boucher de Separación.pdf`, etc.) con la extensión real del archivo; sufijo `(2)`, `(3)` si hay nombres repetidos.
