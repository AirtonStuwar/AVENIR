...
**Columna "Monto Total"** (prop `showMontoTotal` en `SolicitudesTable`, `true` cuando `userRole` es USUARIO o APROBADOR, seteado en `SolicitudesPage.tsx` como `canVerMontoTotal`): muestra `s.monto_total` tal cual está guardado — bruto (con IGV en OC, sin IGV en RxH, ya neteado en Liberalidad), sin descontar detracción ni retención. Es un prop **independiente** de `showMontoPago` (no forma parte de ese grupo) — se agregó en setiembre 2026 solo para USUARIO porque el usuario pidió específicamente verlo, y se extendió a APROBADOR en el mismo mes (mismo criterio, sin quitárselo a USUARIO). VISUALIZADOR/ADMIN no la ven — para ellos sigue siendo exclusivo "Monto a pagar"/"Detracción/Retención".
