...
- `EVALUADOR` — cola En Revision, promedio de días de espera, lista de más antiguas con alerta ≥3 días de Solicitudes, más 4 filas de KPIs "por evaluar vs evaluado" (S/ y $) para Solicitudes, A Rendir, Reembolso y Devolución de Cliente, y tabla "Evaluadas" (código, proveedor/razón social, empresa, fecha) al final. Usa `getEvaluadorData()`, que ahora también trae A Rendir (`En Revision`/`Cerrado`), Reembolso y Devolución (`En Revision`/`Evaluado`). Para A Rendir el monto "en cola/cerrado" usa `total_reembolso` (gasto real ya rendido), no `importe` (adelanto) — helper `montoARendirGasto()` en `DashboardPage.tsx`, distinto de `montoARendir()` que usa `importe` para los demás paneles.

**KPI "Evaluadas" (Solicitudes) es global, no por evaluador:** cuenta todas las solicitudes en estado `Evaluado` del sistema, sin importar qué evaluador las evaluó — a propósito, para que cualquier evaluador vea el total del equipo, no solo lo suyo. Esto es **distinto** al filtro "Evaluado" de la lista de Solicitudes (`setEstadoNombre`), que sí restringe a `usuario_evaluador === userId` (solo lo que ese evaluador evaluó) — si el dashboard muestra 11 y la lista filtrada muestra 9, es porque hay más de un evaluador activo; no es un bug. `SOL_SELECT` en `dashboardService.ts` incluye `usuario_evaluador` por si se necesita en el futuro, aunque el KPI actual no lo usa para filtrar.
