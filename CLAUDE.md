...
**Orden de columnas en `SolicitudesTable`:** Código, Razón social, **Estado**, Factura, [**Monto Total** — solo USUARIO, ver abajo], [**Monto a pagar**, **Detracción/Retención** — solo VISUALIZADOR/ADMIN, ver abajo], Empresa, Fecha pedido, Vencimiento, Creado por, Área, **RUC** — Estado y RUC intercambiaron posiciones respecto al orden original (RUC ahora al final, junto a las acciones).

**Columna "Monto Total"** (prop `showMontoTotal` en `SolicitudesTable`, `true` solo cuando `userRole === ROLES.USUARIO`, seteado en `SolicitudesPage.tsx` como `canVerMontoTotal`): muestra `s.monto_total` tal cual está guardado — bruto (con IGV en OC, sin IGV en RxH, ya neteado en Liberalidad), sin descontar detracción ni retención. Es un prop **independiente** de `showMontoPago` (no forma parte de ese grupo) — se agregó en setiembre 2026 porque el usuario pidió específicamente verlo, a diferencia de "Monto a pagar" que sigue siendo exclusivo de VISUALIZADOR/ADMIN.
